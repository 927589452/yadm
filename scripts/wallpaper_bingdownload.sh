...
saveDir=$HOME'/wallpapers/BingDesktopImages/'
...
desiredPicURL=$bing$(echo $(curl -s $xmlURL) | grep -oE "<urlBase>(.*)</urlBase>" | cut -d ">" -f 2 | cut -d "<" -f 1)$desiredPicRes$picExt

# Form the URL for the default pic resolution
defaultPicURL=$bing$(echo $(curl -s $xmlURL) | grep -oE "<url>(.*)</url>" | cut -d ">" -f 2 | cut -d "<" -f 1)
